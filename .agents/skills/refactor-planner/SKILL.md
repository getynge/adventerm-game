...
- **Read current code; do not rely on memory.** `agents/` docs and `AGENTS.md` describe the architecture; the working tree describes the actual shape today. They diverge whenever exploration code outpaces the docs — trust the tree.
- **Stop at every `AGENTS.md` hard rule.** A refactor is the *right* time to bring violations back into line (lib/binary boundary, no `ratatui`/`crossterm` in the library, append-only enum discriminants, FFI sync requirement, no UI state in the lib) — but never to introduce a *new* violation.
...
- [/Users/getynge/code/adventerm-game/AGENTS.md](../../../AGENTS.md) — workspace rules.
- [adventerm_lib/AGENTS.md](../../../adventerm_lib/AGENTS.md), [adventerm/AGENTS.md](../../../adventerm/AGENTS.md), [adventerm_ffi/AGENTS.md](../../../adventerm_ffi/AGENTS.md) — per-crate rules.
...
- Which `AGENTS.md` rules it currently violates, if any.
...
- **God struct** — fields that belong to different lifetimes, owners, or screens cohabiting one type. Replacement: split per-screen state to the screen variant that uses it (AGENTS.md style rule #1 + [agents/architecture.md](../../../agents/architecture.md)'s screen FSM).
- **Magic number** — inline literal for a size, padding, range, duration, or capacity. Replacement: named constant in the file's existing constants block, or in [adventerm/src/ui/layout.rs](../../../adventerm/src/ui/layout.rs) for layout literals (AGENTS.md style rule #2).
...
- **Boundary leak** — `KeyCode`, `Color`, `Rect`, or any `ratatui`/`crossterm` type in the library; or `World`, `Dungeon`, `Lighting`, `ItemSubsystem` reached for from the binary. Replacement: route through the existing facade (`GameState::tile_at`, `peek_item_here`, …) or add a new lib facade if none fits (AGENTS.md architectural rule #2).
- **Inlined helper** — a non-trivial computation expressed inline at a call site that has its own name in the team's vocabulary. Replacement: small named function near the call site (AGENTS.md style rule #3).
...
- **Style preferences.** Anything in the user's [global AGENTS.md](file:///Users/getynge/.Codex/AGENTS.md) or the project's [AGENTS.md](../../../AGENTS.md) that the current code violates and the refactor should restore.

Note any guideline that contradicts a `AGENTS.md` rule and surface the conflict before continuing.
...
3. **Hard constraints** — the `AGENTS.md` rules being honored or restored, restated.
...
Reference `agents/` docs and `AGENTS.md` by relative path, exactly as [feature-planner](../feature-planner/SKILL.md) and [sync-ffi](../sync-ffi/SKILL.md) do.
...
- Any `AGENTS.md` rule the refactor restores (e.g. "phase 3 removes the last `KeyCode` import from `adventerm_lib`, restoring architectural rule #2").
