...
This skill walks a feature from "rough idea" to a phased, durable plan committed under `plans/<slug>/`. It is the entry point for any change large enough that diving straight to code would lose intent or skip a hard rule from [AGENTS.md](../../../AGENTS.md).
...
- **Read current code; do not rely on memory.** `agents/` docs and `AGENTS.md` are the source of truth for architecture; `git log` and the working tree are the source of truth for recent state.
- **Stop at every `AGENTS.md` hard rule.** The lib/binary boundary, append-only enum discriminants, FFI sync requirement, no `ratatui`/`crossterm` in `adventerm_lib`, no UI state in the lib — these are absolute. If the feature appears to require breaking one, raise it with the user before writing the plan.
...
- The hard rules from `AGENTS.md` that constrain this feature.
...
- Style preferences and what to avoid (matches against project [AGENTS.md](../../../AGENTS.md) and global standards).
...
3. **Hard constraints** — the `AGENTS.md` rules this feature must respect, restated.
...
Reference `agents/` docs and `AGENTS.md` by relative path, exactly as `sync-ffi` does.
...
- Anything that touched a `AGENTS.md` hard rule and needs a design discussion before implementation begins.
