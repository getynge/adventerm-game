...
- `translate_text(event) -> Option<TextInputAction>` — only used in `NameEntry`
...
- [gameplay.rs](../adventerm/src/ui/gameplay.rs) — three-region layout per CLAUDE.md rule #3 (world, dialog beneath, actions panel right). `scroll_offset` keeps the player centered. Tile glyphs: `@` player, `.` floor, `#` wall, `+` door.
...
- [colors.rs](../adventerm/src/ui/colors.rs) — `MenuColors`, `WorldColors`, `SchemeColors`; `menu_block(title, colors)` returns a styled bordered `Block`; `rgb_to_color` converts a palette `Rgb` to ratatui `Color`
