...
- `adventerm_lib` — gameplay logic and state. No TUI dependency. See [adventerm_lib/AGENTS.md](adventerm_lib/AGENTS.md).
- `adventerm` — the TUI binary (`ratatui` 0.30 / `crossterm` 0.29). Renders state, forwards input. See [adventerm/AGENTS.md](adventerm/AGENTS.md).
- `adventerm_ffi` — C-compatible FFI surface over `adventerm_lib` for non-Rust hosts (Swift on iOS, etc.). See [adventerm_ffi/AGENTS.md](adventerm_ffi/AGENTS.md).
